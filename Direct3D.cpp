...
namespace Direct3D
{
	...
	}shaderBundle[SHADER_MAX];	//各シェーダー情報
...
HRESULT Direct3D::Initialize(int winW, int winH, HWND hWnd)
{
	...
	//その他
	scDesc.Windowed = TRUE;			//ウィンドウモード(TRUE)かフルスクリーン(FALSE)か
...
{
	//それぞれをデバイスコンテキストにセット
	pContext_->VSSetShader((shaderBundle[_type].pVertexShader_), NULL, 0);	//頂点シェーダー
	pContext_->PSSetShader((shaderBundle[_type].pPixelShader_), NULL, 0);	//ピクセルシェーダー
	pContext_->IASetInputLayout((shaderBundle[_type].pVertexLayout_));	//頂点インプットレイアウト
	pContext_->RSSetState((shaderBundle[_type].pRasterizerState_));		//ラスタライザー
...
void Direct3D::Release()
{
	for (int i = 0; i < SHADER_MAX; i++) {
		SAFE_RELEASE((shaderBundle[SHADER_3D].pRasterizerState_));
		SAFE_RELEASE((shaderBundle[SHADER_3D].pVertexLayout_));
		SAFE_RELEASE((shaderBundle[SHADER_3D].pPixelShader_));
		SAFE_RELEASE((shaderBundle[SHADER_3D].pVertexShader_));
	}
